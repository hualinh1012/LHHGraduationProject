 ...
 * @author Linh Hua
 */
public class UploadAvatarAPI implements IApiAdapter {
    ...
    public ServerResponse execute(ClientRequest request) {
        ...
        try {
            ...
            String mimeType = request.file.getContentType();
            if (mimeType.contains("image")){                
                byte[] arrayImageInput = FileHandler.getInputArrayByte(request.file.getInputStream());
                String fileUrl = FileHandler.generateFile(arrayImageInput, fileName, fileType);
